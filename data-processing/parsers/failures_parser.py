...
class FailuresParser:
    """Parser for failures markdown file with date-based entries"""
    ...
    def parse_failures_file(self, file_path):
        """Parse failures markdown file and extract date-based entries"""
        file_path = Path(file_path)
        
        if not file_path.exists():
            print(f"Failures file not found at {file_path}")
            return []
        ...
        failures.sort(key=lambda x: (x["date"] == "Unknown", x["date"]), reverse=True)
        
        # Reassign IDs after sorting
        for idx, failure in enumerate(failures, start=1):
            failure["id"] = idx

        print(f"Parsed {len(failures)} failures from {file_path.name}")
        return failures
